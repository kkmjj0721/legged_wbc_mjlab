...
RI-4438 HIM Rough 的随机障碍现采用 mjlab 内置高度场：保留 20% 采样比例、0.4 m 障碍尺寸和 0.01–0.12 m 的高度幅值课程，每块地形随机放置 100 次凸起或凹坑（允许重叠），保留中央 1 m 平台。7 种楼梯配置保持原样。高度场的障碍分布和边缘形状与原方块网格不同，训练难度需要重新评估。
