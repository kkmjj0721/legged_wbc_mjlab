...
class LocomotionFSM:
    ...
    def __init__(self, cfg: SimConfig, backend: RobotBackend, policy: OnnxPolicy, commands: CommandBus):
        ...
        self._stand_start: RobotState | None = None
    ...
    def step(self) -> None:
        ...
        if self.state == State.PASSIVE:
            ...
            assert self._stand_start is not None
            amount = min(1.0, self.state_time / max(self.cfg.stand_duration, 1e-3))
            position = (1 - amount) * self._stand_start.position + amount * self.cfg.standing_pos
            quaternion = quaternion_slerp(self._stand_start.quaternion, self.cfg.standing_quat, amount)
            self.target = (1 - amount) * self._stand_start.joint_position + amount * self.cfg.default_joint
            ...
            if self.fallen(state) or not getattr(self.backend, "finite", lambda: True)():
                self._request_passive(state)
